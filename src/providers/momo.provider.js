...
const normalizeAmount = (amount) => {
  ...
  if (!Number.isFinite(rate) || rate <= 0) throw new HttpError(500, 'MOMO_CONVERT_RATE is invalid');
  const converted = Math.round(num * rate);
  if (!Number.isFinite(converted) || converted <= 0) throw new HttpError(400, 'MoMo amount is invalid');
  return converted;
};
...
const requestJson = (targetUrl, payload) =>
  new Promise((resolve, reject) => {
    ...
    req.on('error',   (err) => reject(new HttpError(502, err.message || 'Cannot connect to MoMo')));
    req.write(data);
    req.end();
  });
