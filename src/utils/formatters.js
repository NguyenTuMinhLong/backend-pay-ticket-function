...
const buildPaymentInstruction = ({ payment, providerPayload = {}, bankConfig, sepayConfig, momoConfig }) => {
  ...
  if (method === 'MOMO') {
    return {
      ...
      redirect_url: momoConfig.redirectUrl || null,
      ipn_url: momoConfig.ipnUrl || null,
      auto_confirm_ready: true,
      auto_confirm_note: 'MoMo sẽ redirect người dùng về /payments/momo/return và gọi IPN về /payments/momo/ipn khi thanh toán xong.',
      note: 'Frontend có thể mở pay_url hoặc deeplink để chuyển người dùng sang MoMo.',
    };
  }
